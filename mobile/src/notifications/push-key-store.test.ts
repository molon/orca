...
import { deletePushKey, loadPushKey, savePushKey } from './push-key-store'
...
describe('push key store', () => {
  ...
  it('stores the key so the extension can read it on a locked phone', async () => {
    await savePushKey('host-1', 'key-b64')
  ...
  it('reads back with the same options it wrote with', async () => {
    getItemAsync.mockResolvedValueOnce('key-b64')
    await expect(loadPushKey('host-1')).resolves.toBe('key-b64')
    expect(getItemAsync.mock.calls[0]![0]).toBe(
      setItemAsync.mock.calls[0]?.[0] ?? 'orca:push-key:host-1'
    )
  ...
  it('scopes keys per host, so unpairing one host cannot read another key', async () => {
    await savePushKey('host-1', 'a')
    await savePushKey('host-2', 'b')
    expect(setItemAsync.mock.calls[0]![0]).not.toBe(setItemAsync.mock.calls[1]![0])
  })

  it('encodes the host id so an exotic id cannot collide or escape its key', async () => {
    await savePushKey('host/../other', 'a')
    expect(setItemAsync.mock.calls[0]![0]).not.toContain('/')
  })

  it('returns null when no key is stored', async () => {
    getItemAsync.mockResolvedValueOnce(null)
    await expect(loadPushKey('host-1')).resolves.toBeNull()
  })
  ...
  it('does not throw when deleting a key that cannot be removed', async () => {
    ...
    await expect(deletePushKey('host-1')).resolves.toBeUndefined()
  })
})

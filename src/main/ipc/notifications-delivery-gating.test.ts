...
describe('registerNotificationHandlers', () => {
  ...
  it('opens the current macOS app notification settings entry', async () => {
    ...
    process.env.ORCA_DEV_MACOS_BUNDLE_ID = 'com.molon.orca.dev.fb5a47066f08'
    try {
      registerNotificationHandlers({
        getSettings: () => ({
          notifications: {
      ...
      expect(shellOpenExternalMock).toHaveBeenCalledWith(
        'x-apple.systempreferences:com.apple.Notifications-Settings.extension?id=com.molon.orca.dev.fb5a47066f08'
      )
    } finally {
